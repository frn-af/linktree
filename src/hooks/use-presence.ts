	...
	return useQuery<PresenceSession[]>({
		...
		queryFn: async () => {
			const res = await fetch(
				`/api/presence/sessions${showAll ? "?all=true" : ""}`,
			);
...
export function useUpdatePresenceSession() {
	...
	return useMutation({
		mutationFn: async (
			updatedSession: Partial<PresenceSession> & { id: string },
		) => {
			const res = await fetch("/api/presence/sessions", {
				method: "PUT",
				headers: { "Content-Type": "application/json" },
...
// Records
export function usePresenceRecords(
...
export function useUpdatePresenceRecord() {
	...
	return useMutation({
		mutationFn: async (
			updatedRecord: Partial<PresenceRecord> & { id: string },
		) => {
			const res = await fetch("/api/presence", {
				method: "PUT",
				headers: { "Content-Type": "application/json" },
